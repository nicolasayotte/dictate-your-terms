...
|--------------|-------------------|-----------------------------------|-------------|
| `stt-daemon` | Inference Engine  | axum, tokio, whisper-rs, hound    | Persistent background server. Holds the STT model hot in RAM/VRAM, eliminating cold starts. Exposes a single local API endpoint (`POST /transcribe`). |
| `stt-cli`    | Capture Client    | cpal, reqwest, arboard            | Transient CLI binary. Captures raw f32 audio from the OS default mic, fires it to the daemon, dumps returned text to the system clipboard, and exits. |
...
2. A terminal split briefly opens, running `stt-cli --record`.
3. `cpal` captures your voice detailing a complex architectural constraint.
4. You hit Enter to end the capture.
5. `stt-cli` fires the payload to `stt-daemon` (running `whisper-rs`).
6. The daemon responds in ~400ms.
7. `stt-cli` pipes the text to the clipboard and exits.
